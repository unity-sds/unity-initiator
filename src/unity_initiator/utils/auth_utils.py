...
class TokenManager:
    ...
    def __init__(self, username: str, password: str, client_id: str, region: str = "us-west-2"):
    ...
    def get_valid_token(self) -> Optional[str]:
        ...
        if (self._token_cache and 
            self._token_cache.expires_at > current_time + self._refresh_buffer):
    ...
    def _refresh_token(self) -> Optional[str]:
        ...
        if not self._token_cache or not self._token_cache.refresh_token:
            logger.warning("No refresh token available, fetching new token with credentials")
            return self._fetch_new_token()
        ...
        try:
            with httpx.Client(verify=False) as client:  # nosec
                ...
                if "AuthenticationResult" in result:
                    ...
                    logger.warning("Refresh token failed, fetching new token with credentials")
                    return self._fetch_new_token()
                    
        except (httpx.HTTPStatusError, httpx.RequestError) as e:
            logger.warning("Token refresh failed: %s, fetching new token with credentials", str(e))
            return self._fetch_new_token()
